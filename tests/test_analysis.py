...
from gedcom_mcp.operations import GetAncestorsParams, GetDescendantsParams, GetStatsParams
...
async def test_handle_get_ancestors(fixtures_dir: Path) -> None:
    ...
    result = _text(
        await handle_get_ancestors(ctx, GetAncestorsParams(xref="@I9@", max_generations=5))
    )
...
async def test_handle_get_ancestors_full_chain(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(
        await handle_get_ancestors(ctx, GetAncestorsParams(xref="@I9@", max_generations=10))
    )
...
async def test_handle_get_ancestors_no_parents(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(
        await handle_get_ancestors(ctx, GetAncestorsParams(xref="@I1@", max_generations=5))
    )
...
async def test_handle_get_ancestors_not_found(fixtures_dir: Path) -> None:
    ...
    with pytest.raises(McpToolError, match="not found"):
        await handle_get_ancestors(ctx, GetAncestorsParams(xref="@I999@", max_generations=5))


async def test_handle_ancestors_default_depth(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(await handle_get_ancestors(ctx, GetAncestorsParams(xref="@I9@")))
...
async def test_handle_get_descendants(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(
        await handle_get_descendants(ctx, GetDescendantsParams(xref="@I1@", max_generations=5))
    )
...
async def test_handle_get_descendants_full_chain(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(
        await handle_get_descendants(ctx, GetDescendantsParams(xref="@I1@", max_generations=10))
    )
    assert "Thomas /Adams/" in result
...
async def test_handle_get_descendants_no_children(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(
        await handle_get_descendants(ctx, GetDescendantsParams(xref="@I9@", max_generations=5))
    )
...
async def test_handle_get_descendants_not_found(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    with pytest.raises(McpToolError, match="not found"):
        await handle_get_descendants(ctx, GetDescendantsParams(xref="@I999@", max_generations=5))


async def test_handle_descendants_capped_by_max(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(
        await handle_get_descendants(ctx, GetDescendantsParams(xref="@I1@", max_generations=1))
    )
...
async def test_handle_get_stats_medium(fixtures_dir: Path) -> None:
    ctx = _make_ctx(fixtures_dir, "medium.ged")
    result = _text(await handle_get_stats(ctx, GetStatsParams()))
...
async def test_handle_get_stats_no_database() -> None:
    ...
    with pytest.raises(McpToolError, match="No GEDCOM file is loaded"):
        await handle_get_stats(ctx, GetStatsParams())
